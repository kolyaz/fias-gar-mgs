...
import Unzippo from 'unzippo';
import progress_stream from 'progress-stream';
import xmlToolkit from 'xml-toolkit';
import GarXmlObjectTransformer from './GarXmlObjectTransformer.js';
import GarXmlTabTextTransformer from './GarXmlTabTextTransformer.js';

const { XMLLexer, SAXEvent } = xmlToolkit;
const { list, open } = Unzippo;
const GarXmlZip = class {
  ...
  async getDate() {
    const dir = await list(this.path);
  ...
  async getDataDictionary({ name, filter, map } = {}) {
    ...
    strictEqual(typeof name, 'string', '`name` must be defined as string');
    ...
    const m = new Map(); const
      lex = (await open(this.path, path)).pipe(new XMLLexer());
  ...
  async createReadStream({ name, region, filter, map, join, progress } = {}) {
    try {
      ...
      let is = await open(this.path, path);
